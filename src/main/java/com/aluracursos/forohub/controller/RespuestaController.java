...
public class RespuestaController {
    ...
    public ResponseEntity<?> listarRespuestaPorTopico(
        ...
        if (topicoId != null) {
            ...
            if (!respuestas.isEmpty()) {
                ...
                return ResponseEntity.status(HttpStatus.CONFLICT).body("No hay resultados del topico buscado.");
            }
        } else {
    ...
    public ResponseEntity actualizarTopico(@RequestBody @Valid DatosRegistroRespuestas.DatosActualizarRespuestas datosActualizarRespuesta) {
        ...
        if (respOptional.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.CONFLICT).body("No hay respuesta con el id seleccionado.");
        }
    }
    ...
    public ResponseEntity eliminarRespuesta(@PathVariable Long id) {
        ...
        if (respuestaOptional.isPresent()) {
            ...
            return ResponseEntity.ok().body("Respuesta eliminada exitosamente");
        }
    ...
    public ResponseEntity darDeBajaRespuesta(@PathVariable Long id) {
        ...
        if (respuestaOptional.isPresent()) {
            ...
            return ResponseEntity.ok().body("Respuesta dada de baja exitosamente");
        }
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity marcarComoSolucion(@PathVariable Long id) {
        ...
        if (respuestaOptional.isPresent()) {
            ...
            if (topico.getEstaSolucionado().equals(Boolean.FALSE)) {
                ...
                return ResponseEntity.ok().body("Respuesta marcada como solución al tópico");
            } else {
                return ResponseEntity.status(HttpStatus.CONFLICT).body("Tópico solucionado.");
            }
        }
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity retornarDatosRespuesta(@PathVariable Long id) {
        ...
        if (optionalResp.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No hay respuestas con el ID solicitado.");
        }
    }
}
